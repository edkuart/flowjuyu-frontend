...
function reducer(state: ProductEditState, action: Action): ProductEditState {
  switch (action.type) {
    ...
    case "IMAGE_REMOVED": {
      if (!state.data) return state
      const removed = state.data.imagenes.find((img) => img.id === action.imageId)
      ...
      return {
        ...
        data: {
          ...state.data,
          imagenes: state.data.imagenes.filter((img) => img.id !== action.imageId),
...
function normalizeProductEditData(raw: Record<string, unknown>): ProductEditData {
  return {
...
const FORMDATA_SKIP = new Set(["imagenes", "imagen_principal", "created_at", "id"])
...
export function useProductEdit(productId: string): UseProductEditReturn {
  ...
  useEffect(() => { productRef.current  = state.data    }, [state.data])
  useEffect(() => { isSavingRef.current = state.isSaving }, [state.isSaving])
  ...
  const load = useCallback(async (silent = false) => {
    ...
    try {
      ...
      if (!res.ok) {
        ...
        throw new Error(body.message ?? `Error ${res.status} al cargar producto`)
      }
  ...
  const saveSection = useCallback(
    async (sectionId: string) => {
      ...
      if (validationError) {
        dispatch({ type: "SAVE_ERROR", section: sectionId, error: validationError })
        return
      }
  ...
  const uploadImages = useCallback(
    async (files: File[]) => {
      ...
      const validationError = validateProduct(product)
      if (validationError) {
        dispatch({ type: "SAVE_ERROR", section: "imagenes", error: validationError })
        return
      }
      ...
      try {
        ...
        if (!res.ok) {
          const body = await res.json().catch(() => ({})) as { message?: string }
          throw new Error(body.message ?? `Error ${res.status} al subir imágenes`)
        }
  ...
  const deleteImage = useCallback(async (imageId: number) => {
    ...
    const res = await apiFetch(
      `/api/productos/${product.id}/imagenes/${imageId}`,
      { method: "DELETE" }
    )
    if (!res.ok) {
      const body = await res.json().catch(() => ({})) as { message?: string }
    ...
    dispatch({ type: "IMAGE_REMOVED", imageId })
  }, [])
  ...
  const setPrincipalImage = useCallback(async (imageUrl: string) => {
    ...
    const res = await apiFetch(
      `/api/productos/${product.id}/set-principal`,
      {
        method: "PATCH",
  ...
  const getSectionState = useCallback(
    (id: string): SectionSaveState =>
      state.sectionStates[id] ?? IDLE_SECTION,
    [state.sectionStates]
  )
