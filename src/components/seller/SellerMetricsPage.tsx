// src/components/seller/SellerMetricsPage.tsx
...
export default function SellerMetricsPage() {
  ...
  const priorityInsight = useMemo((): PriorityInsight => {
    ...
    if (analytics.totalProductViews === 0) {
      return {
        ...
        description:
          "Comparte el enlace de tu tienda para empezar a atraer clientes. Sin visitas, ninguna otra métrica puede mejorar.",
    ...
    if (
      ...
      return {
        ...
        description:
          "Tienes visitas pero ningún cliente ha hecho click en tu WhatsApp. Verifica que tu número esté activo y visible.",
    ...
    if (analytics.totalReviews === 0 && analytics.totalProductViews >= 15) {
      return {
        ...
        description:
          "Las reseñas generan confianza y aumentan la conversión. Pídele a tus clientes que dejen su opinión.",
    ...
    return {
      ...
      description:
        "No hay problemas críticos detectados. Sigue optimizando para seguir creciendo.",
  ...
  const reviewValue =
    analytics.totalReviews > 0 && reviewAverage > 0
  ...
  return (
    <main className="mx-auto min-h-screen max-w-6xl space-y-6 bg-[#f8f5ef] px-4 py-8 sm:px-6">
      {loading ? (
        <div className="space-y-5">
          ...
          <div className="grid grid-cols-1 gap-4 md:grid-cols-3">
            {[0, 1, 2].map((i) => (
              <div
                key={i}
                className="h-44 animate-pulse rounded-2xl bg-white/70"
              />
            ))}
          </div>
        </div>
        ...
        <>
              ...
              >
                <div className="flex flex-col gap-4 md:flex-row md:items-center md:justify-between">
                  ...
                  <div className="flex flex-wrap gap-3">
                    <Link href={onboardingSummary.ctaHref}>
                      <Button className="min-h-10 px-4">
                        {onboardingSummary.ctaLabel}
                      </Button>
                    </Link>
          ...
          <BaseSection>
            <BaseSectionHeading
              eyebrow="Metricas"
              title="Rendimiento de tu negocio"
              description="Tus metricas viven en una ruta independiente del onboarding y muestran contexto util aunque todavia estes activando la tienda."
            />
...
function PriorityInsightBanner({ insight }: { insight: PriorityInsight }) {
  ...
  return (
    <BaseCard hover className={styles.border} contentClassName="space-y-3">
      <div className="space-y-3">
        <span
          className={`inline-flex items-center self-start rounded-full px-2.5 py-1 text-[11px] font-semibold ${styles.badge}`}
        >
          {SEVERITY_LABELS[insight.severity]}
        </span>
        <div className="min-w-0 space-y-2">
          <h2 className="text-lg leading-snug font-semibold text-neutral-900 sm:text-xl">
            {insight.title}
          </h2>
  ...
  return (
    ...
    >
      ...
      <div className="min-w-0 space-y-2.5">
        ...
        <p
          className={`text-3xl leading-none font-bold sm:text-[2rem] ${valueColor}`}
        >
          {value}
        </p>
      </div>
      {interpretation && <InterpBadge interp={interpretation} />}
      <p className="line-clamp-3 min-w-0 text-sm leading-relaxed text-neutral-600">
        {context}
      </p>
    </BaseCard>
  );
        ...
        <InfoTile
          ...
          detail={
            reviewInsights.low_rating_count > 0
              ? "Hay señales que vale la pena revisar pronto."
